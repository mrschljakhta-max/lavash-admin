...
function setStatusBadge(text, mode = 'default') {
  ...
  if (mode === 'ok') node.classList.add('is-ok');
  if (mode === 'warn') node.classList.add('is-warn');
...
async function loadPendingRows() {
  setStatusBadge('Завантаження...', 'warn');
  ...
  if (error) {
    ...
    setStatusBadge('Помилка завантаження', 'error');
    return;
  }
...
function bindLeftNavRoutes() {
  ...
  $all('.nav-item').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (window.location.pathname.endsWith(href.replace('./', ''))) return;
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    setStatusBadge('Критична помилка', 'error');
  }
});
