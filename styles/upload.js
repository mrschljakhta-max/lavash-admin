  ...
  modal.innerHTML = `
    ...
    <div class="upload-details-modal__box">
      <div class="upload-details-modal__head">
        ...
        <button data-upload-details-close="1">×</button>
      </div>
      <div id="uploadDetailsBody"></div>
    </div>
  `;
  document.body.appendChild(modal);

  modal.addEventListener('click', (e)=>{
    if(e.target.dataset.uploadDetailsClose) modal.classList.add('hidden');
  });
...
function openUploadDetailsModal(title, html) {
  ...
  document.getElementById('uploadDetailsTitle').textContent = title;
  document.getElementById('uploadDetailsBody').innerHTML = html;
  modal.classList.remove('hidden');
}

window.LavashUpload = window.LavashUpload || {};
window.LavashUpload.showLastUploadSummary = function(){
  openUploadDetailsModal("Останній batch", "<pre>"+JSON.stringify(window.lastBatch || {}, null, 2)+"</pre>");
};
